...
For the Tenth edition, we have prioritized testing our developments
with the stable Long Term Support (``LTS'') versions of the
\textsc{OpenJDK}, namely \textsc{openjdk8}, \textsc{openjdk11}, and
\textsc{openjdk17}.  We intend to drop one or more of these platforms
...
outside the hosting \textsc{JVM}.  As such, the Tenth edition is built
for release with openjdk8 but should run best on \textsc{openjdk17}.
...
(let ((jar (jnew "java.util.zip.ZipFile" "/path/to/some.jar")))
  (let ((entries (jcall "entries" jar)))
    (jcall "hasMoreElements" entries)))
\end{listing-lisp}
...
\cite{pandemos}, we dyslexic worker bears untied abcl-1.9.0 into the
future.
